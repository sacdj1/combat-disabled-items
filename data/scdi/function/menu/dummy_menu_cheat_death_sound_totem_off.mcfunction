# per-dummy setting - see load.mcfunction's dummy_cheat_death_sound_totem
...
execute at @s if data storage scdi:config {allow_dummy_trigger:1b} as @e[type=minecraft:mannequin,tag=scdi_dummy,distance=..10,sort=nearest,limit=1] run scoreboard players set @s scdi_dummy_cheat_death_sound_totem 0
execute at @s if data storage scdi:config {allow_dummy_trigger:1b} if entity @e[type=minecraft:mannequin,tag=scdi_dummy,distance=..10] run tellraw @s {"text":"(✔) This dummy no longer plays the totem pop when cheating death.","color":"green"}
