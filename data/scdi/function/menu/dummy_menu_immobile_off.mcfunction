# per-dummy setting (not global) - see menu/dummy_immobile_off.mcfunction
...
execute at @s if data storage scdi:config {allow_dummy_trigger:1b} as @e[type=minecraft:mannequin,tag=scdi_dummy,distance=..10,sort=nearest,limit=1] run attribute @s minecraft:knockback_resistance base set 0.0
execute at @s if data storage scdi:config {allow_dummy_trigger:1b} if entity @e[type=minecraft:mannequin,tag=scdi_dummy,distance=..10] run tellraw @s {"text":"(✔) This dummy now takes real knockback.","color":"green"}
